...
class Program
    {
        ...
        static void PrintDebtors(List<Debtor> debtors, string title)
        {
        ...
        static bool CanFormWord(string input, string wordToForm)
        {
            foreach (char letter in wordToForm)
            {
        ...
        static int CalculateMonthsUntilBirthday(DateTime birthDay)
        {
        ...
        static void Main()
        {
            ...
            var debtorsWithSpecificDomains = debtors.Where(d => d.Email.EndsWith("@rhyta.com") || d.Email.EndsWith("@dayrep.com")).ToList();
            PrintDebtors(debtorsWithSpecificDomains, "rhyta.com ve ya dayrep.com domenlerinde emaili olan borclular");



            //3) Yashi 26 - dan 36 - ya qeder olan borclulari cixartmag
            var debtorsBetween26And36 = debtors.Where(d => d.BirthDay >= DateTime.Now.AddYears(-36) && d.BirthDay <= DateTime.Now.AddYears(-26)).ToList();
            PrintDebtors(debtorsBetween26And36, " 26 - dan 36 - ya qeder olan borclular");


            //4) Borcu 5000 - den cox olmayan borclularic cixartmag

            var debtorsWithDebtLessThan5000 = debtors.Where(d => d.Debt < 5000).ToList();
            PrintDebtors(debtorsWithDebtLessThan5000, "5000 - den cox olmayan borclular");



            //5) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2 - den cox 7 reqemi olan borclulari cixartmaq

            var debtorsWithLongNameAndSpecificDigits = debtors.Where(d => d.FullName.Length > 18 && (d.Phone.Contains("2") || d.Phone.Count(char.IsDigit) >= 2)).ToList();
            PrintDebtors(debtorsWithLongNameAndSpecificDigits, "");



            //7) Qishda anadan olan borclulari cixardmaq
            var winterBornDebtors = debtors.Where(d => d.BirthDay.Month == 12 || (d.BirthDay.Month >= 1 && d.BirthDay.Month <= 2)).ToList();
            PrintDebtors(winterBornDebtors, "Qishda anadan olan borclular");
            ...
            var debtorsAboveAvgDebt = debtors.Where(d => d.Debt > avgDebt).OrderBy(d => d.FullName).ThenByDescending(d => d.Debt).ToList();
            PrintDebtors(debtorsAboveAvgDebt, "Borcu orta səviyyədən yuxarı olan borclular, adlara və azalan borclara görə sıralanır");
            ...
            var debtorsWithThreeSameLetters = debtors.Where(d => d.FullName.Distinct().Count() == 3 && d.FullName.All(char.IsLetter) && d.FullName.Length > 0)
                                                  .OrderBy(d => d.FullName).ToList();
            PrintDebtors(debtorsWithThreeSameLetters, "Adında üç eyni hərf olan borclular əlifba sırası ilə sıralanır");

            //13)borclulardan en coxu hansi ilde dogulubsa hemin ili cixartmaq

            var mostBornInYear = debtors.GroupBy(d => d.BirthDay.Year).OrderByDescending(g => g.Count()).First().Key;
            Console.WriteLine($"Borcluların çoxu ildə anadan olub {mostBornInYear}");

            //14)Borcu en boyuk olan 5 borclunun siyahisini cixartmaq
            var top5Debtors = debtors.OrderByDescending(d => d.Debt).Take(5).ToList();
            PrintDebtors(top5Debtors, "Borca görə ilk 5 borclu");

            //15)Butun borcu olanlarin borcunu cemleyib umumi borcu cixartmaq

            var totalDebt = debtors.Sum(d => d.Debt);
            Console.WriteLine($"Bütün borcluların ümumi borcu: {totalDebt}");
            //16)2ci dunya muharibesin gormush borclularin siyahisi cixartmaq
            var ww2Veterans = debtors.Where(d => d.BirthDay.Year <= 1945).ToList();
            PrintDebtors(ww2Veterans, "İkinci Dünya Müharibəsini yaşamış borclular");
            ...
            var debtorsWhoCanPayOff = debtors.Where(d => (totalDebt - d.Debt) / 500 <= CalculateMonthsUntilBirthday(d.BirthDay)).ToList();
            PrintDebtors(debtorsWhoCanPayOff, "Doğum gününə qədər borcunu ödəyə bilən borclular");


            //20)Adindaki ve familyasindaki herflerden "smile" sozunu yaza bileceyimiz borclularin siyahisini cixartmaq
            var debtorsWithSmileWord = debtors.Where(d => CanFormWord(d.FullName.ToLower(), "smile") || CanFormWord(d.Address.ToLower(), "smile")).ToList();
            PrintDebtors(debtorsWithSmileWord, "Debtors who can form the word 'smile'");
